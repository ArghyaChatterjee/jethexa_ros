...
class ColorKickNode:
    def __init__(self, target_color, log_level=rospy.INFO):
        ...
        self.pid_pitch = pid.PID(0.6, 0.01, 0.01) # pid controller that controls the pitch angle
        self.pid_yaw = pid.PID(0.6, 0.01, 0.01) # pid controller that controls the yaw angle
        ...
        try:
            ...
            if max_contour_area is not None:
                ...
                if abs(center_y - (h / 2)) > 30: #If the difference range is less than a certain value, there is no need to move any more.
                    ...
                    self.pitch += self.pid_pitch.output # acquire pid output
                else:
