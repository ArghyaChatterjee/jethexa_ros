...
class TagNode:
    ...
    def image_proc(self, rgb_image, result_image):
        ...
        if detections:
            for detection in detections: # Traverse all the recognized tags
                tag_id = detection['id']
        ...
        if self.tag is not None:
            ...
            if abs(w - 20) > 2:
                ...
                print(self.speed)
                if self.speed > 0:
                    self.jethexa.set_step_mode(1, abs(self.speed), 10, 0, 0, 0.6, repeat=0)
                elif self.speed < 0:
                    self.jethexa.set_step_mode(1, abs(self.speed), 10, math.radians(180), 0, 0.6, repeat=0)
                else:
                    self.jethexa.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
        else:
            self.jethexa.set_step_mode(1, 0, 10, 0, 0, 0.6, repeat=0)
